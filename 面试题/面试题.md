...
slave向master发送一个psync请求，告诉master我需要同步数据

master接收到消息后，会生成rdb的快照，然后发给slave

slave载入rdb文件，将状态与master保持一致

master在将期间的保存在缓存的写命令发送给slave
...
slave断开重新上线后，会发送psync命令给maseter

master进行服务器判断，然后进行偏移量判断

如果不一致，会发送一个continue回复，执行部分同步

master将断开后的命令发给slave
